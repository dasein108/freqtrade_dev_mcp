...
class Config(BaseModel):
    ...
    freqtrade_path: Path = Field(
        default=Path.home() / "freqtrade", description="Path to Freqtrade installation"
    )
    strategy_dir: Path = Field(
        default=Path("user_data/strategies"), description="Relative path to strategies directory"
    )
    data_dir: Path = Field(
        default=Path("user_data/data"), description="Relative path to data directory"
    )
    ...
    default_exchange: str = Field(
        default="binance", description="Default exchange for data operations"
    )
    default_stake_amount: float = Field(
        default=100.0, description="Default stake amount for backtesting"
    )
    default_epochs: int = Field(default=100, description="Default number of epochs for hyperopt")
    coingecko_api_key: Optional[str] = Field(
        default=None, description="CoinGecko API key (optional)"
    )
    cache_ttl: int = Field(default=900, description="Cache TTL in seconds (15 minutes)")
    log_level: str = Field(default="INFO", description="Logging level")
    max_concurrent_operations: int = Field(default=3, description="Maximum concurrent operations")
...
def load_config(config_path: Optional[Path] = None) -> Config:
    ...
    # Override with environment variables (the only supported ones are listed here)
    env_mapping = {
    ...
    for env_var, config_key in env_mapping.items():
        if env_value := os.getenv(env_var):
            if config_key.endswith("_path"):
                config_data[config_key] = Path(env_value).expanduser()
            else:
                config_data[config_key] = env_value

    # Not configured: use the working directory or its parent if either holds user_data/
    if "freqtrade_path" not in config_data:
        current_dir = Path.cwd()
        for candidate in (current_dir, current_dir.parent):
            if (candidate / "user_data").is_dir():
                config_data["freqtrade_path"] = candidate
                break
