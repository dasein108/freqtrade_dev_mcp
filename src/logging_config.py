...
NOISY_LOGGERS = ["httpx", "urllib3", "freqtrade", "ccxt", "websockets"]
...
def _ensure_log_dir(log_dir: Path) -> Path:
    """Create the log directory, falling back to the system temp dir if it is not writable."""
    try:
        log_dir.mkdir(parents=True, exist_ok=True)
        return log_dir
    except OSError:
        fallback = Path(tempfile.gettempdir()) / "freqtrade_mcp_logs"
...
def setup_mcp_logging(
    ...
    Console output goes to stderr: stdout carries the MCP stdio protocol and must stay clean.

    Args:
        log_level: Logging level for the console handler
        log_to_file: Whether to log to file
        log_dir: Directory for log files (default: <project>/logs, temp dir if not writable)

    Returns:
        Path to log file if logging to file, None otherwise
    """
    log_dir = _ensure_log_dir(log_dir or DEFAULT_LOG_DIR)
