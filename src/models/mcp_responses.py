...
class DownloadResult(BaseModel):
    ...
    successful_pairs: List[str] = Field(
        default_factory=list, description="Successfully downloaded pairs"
    )
    failed_pairs: List[Dict[str, str]] = Field(
        default_factory=list, description="Failed pairs with error info"
    )
    ...
    cache_files: List[CacheFileInfo] = Field(
        default_factory=list, description="Cache files created"
    )
...
class ReadCandlesResponse(MCPResponse):
    ...
    candle_data: Dict[str, Dict[str, CandleData]] = Field(
        ..., description="Nested candle data by symbol/timeframe"
    )
    successful_reads: List[ReadFileInfo] = Field(
        default_factory=list, description="Successfully read files"
    )
...
class BacktestResponse(MCPResponse):
    ...
    pairs_summary: Dict[str, PairSummary] = Field(
        default_factory=dict, description="Per-pair performance"
    )
...
def create_error_response(
    command: str, error: str, error_type: str = "Error", duration: Optional[float] = None
) -> MCPResponse:
...
def create_success_response(
    command: str, duration: Optional[float] = None, **kwargs
) -> MCPResponse:
    """Create a standardized success response."""
    return MCPResponse(command=command, success=True, duration_seconds=duration, **kwargs)
