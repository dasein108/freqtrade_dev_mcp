...
class DownloadCandlesResponse(MCPBaseResponse):
    ...
    cache_files: List[CacheFileInfo] = Field(
        default_factory=list, description="Created cache files"
    )
...
def create_error_response(
    command: str, error: str, duration: Optional[float] = None
) -> MCPBaseResponse:
    """Create standardized error response."""
    return MCPBaseResponse(command=command, success=False, error=error, duration_seconds=duration)


def create_success_response(command: str, duration: Optional[float] = None) -> MCPBaseResponse:
    """Create standardized success response."""
    return MCPBaseResponse(command=command, success=True, duration_seconds=duration)
