...
try:
    ...
    from .logging_config import MCPLogger, setup_mcp_logging
except ImportError:
...
class FreqtradeMCPServer:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        try:
            ...
            log_file = setup_mcp_logging(log_level=log_level, log_to_file=True)
    ...
    async def send_log_message(
        self, level: LoggingLevel, message: str, data: Optional[Dict] = None
    ):
        """Send MCP-safe log message."""
        try:
            # Check if we have an active request context and session
            if (
                hasattr(self.server, "request_context")
                and self.server.request_context
                and hasattr(self.server.request_context, "session")
                and self.server.request_context.session
            ):
    ...
    def _setup_handlers(self):
        ...
        async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            try:
                ...
                logger.info("Tool %s completed successfully in %.3fs", name, execution_time)
                ...
                # Full traceback stays in the server log; the client only gets the message
                logger.exception("Error executing tool %s after %.3fs", name, execution_time)
    ...
    async def run(self):
        ...
        logger.info("Starting Freqtrade MCP Server")
