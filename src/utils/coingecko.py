...
class CoinGeckoClient:
    ...
    async def get_top_coins_by_market_cap(
        self, limit: int = 15, vs_currency: str = "usd"
    ) -> List[Dict]:
        ...
        if cache_key in self._cache and current_time - self._cache_timestamp < 900:
    ...
    def map_coins_to_trading_pairs(
        self, coins: List[Dict], quote_currency: str = "USDT", exchange: str = "binance"
    ) -> List[str]:
    ...
    async def get_top_trading_pairs(
        self, limit: int = 15, quote_currency: str = "USDT", exchange: str = "binance"
    ) -> List[str]:
        ...
        Returns:
            List of trading pair strings
        """
        coins = await self.get_top_coins_by_market_cap(
            limit * 2
