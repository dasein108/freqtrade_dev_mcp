...
class SearchResultsCommand(BaseCommand):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            return {"command": "search_results", "success": False, "error": str(e)}
    ...
    async def _build_search_index(self):
        ...
        await self.mcp_log(
            "info",
            f"Found {len(backtest_files)} backtest files and {len(hyperopt_files)} hyperopt files",
        )
    ...
    async def _index_backtest_file(self, cursor: sqlite3.Cursor, file_path: Path):
        ...
        for term in search_terms:
            if term:  # Skip empty terms
                cursor.execute(
                    "INSERT INTO search_terms (result_id, term) VALUES (?, ?)", (result_id, term)
                )
    ...
    async def _index_hyperopt_file(self, cursor: sqlite3.Cursor, file_path: Path):
        ...
        extract_result = await self.hyperopt_extractor.execute(
            hyperopt_path=str(file_path), include_trials=False, output_format="summary"
        )
    ...
    async def _enhance_results_with_summaries(
        ...
        for result in results:
            ...
            try:
                if result["result_type"] == "backtest":
                    ...
                    if extract_result.get("success"):
                        data = extract_result.get("data", {})
                        enhanced_result["optimization_summary"] = data.get(
