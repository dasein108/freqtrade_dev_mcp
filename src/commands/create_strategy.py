...
try:
    from .base import BaseCommand, validate_strategy_name
except ImportError:
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).parent.parent))
    from commands.base import BaseCommand, validate_strategy_name
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            # Determine strategy file path (defaults to the configured strategies directory,
            # which is where backtest/hyperopt look for strategies)
            if strategy_path:
                strategy_file_path = self.resolve_workspace_path(strategy_path)
                if strategy_file_path.suffix != ".py":
                    raise ValueError("strategy_path must point to a .py file")
            else:
                strategies_dir = self.config.full_strategy_dir
            ...
            return {
                ...
                "summary": {
                    ...
                    "indicators_count": (
                        len(indicators)
                        if indicators
                        else len(self._get_template_indicators(template))
                    ),
...
'''
    ...
    def _generate_class_docstring(
        self, strategy_name: str, template: str, description: Optional[str]
    ) -> str:
...
{roi_string}
    ...
    def _generate_indicators_code(self, indicators: List[str], template: str) -> str:
        ...
        if not code_lines:
            ...
            code_lines = [indicator_code_map["RSI"], indicator_code_map["MACD"]]
    ...
    def _get_template_config(self, template: str) -> Dict[str, Any]:
        ...
        configs = {
            "basic": {"additional_params": {}},
            "trend": {"additional_params": {"startup_candle_count": 30}},
            "mean_reversion": {"additional_params": {"startup_candle_count": 20}},
            "scalping": {
                "additional_params": {"startup_candle_count": 15, "process_only_new_candles": True}
            },
            "advanced": {
                "additional_params": {
    ...
    def _get_default_roi(self, template: str) -> Dict[str, float]:
        ...
        roi_tables = {
            "basic": {"60": 0.01, "30": 0.02, "0": 0.04},
            "trend": {"120": 0.01, "60": 0.03, "30": 0.06, "0": 0.10},
            "mean_reversion": {"40": 0.01, "20": 0.02, "0": 0.04},
            "scalping": {"15": 0.01, "5": 0.02, "0": 0.03},
            "advanced": {"240": 0.01, "120": 0.02, "60": 0.05, "0": 0.08},
        }
