...
class ExtractHyperoptDataCommand(BaseCommand):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            await self.mcp_log(
                "info", f"Successfully extracted hyperopt data from {hyperopt_file.name}"
            )
    ...
    async def _load_hyperopt_file(self, hyperopt_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                await self.mcp_log(
                    "info", f"Loaded hyperopt file as JSON Lines with {len(trials)} trials"
                )
                ...
                try:
                    ...
                    await self.mcp_log(
                        "info",
                        f"Loaded hyperopt file as JSON with {len(data) if isinstance(data, list) else 'unknown'} trials",
                    )
                    return {"trials": data} if isinstance(data, list) else data
                except (json.JSONDecodeError, UnicodeDecodeError) as e:
                    # Pickle-based .fthypt files (freqtrade < 2021) are not supported:
                    # unpickling a user-supplied file can execute arbitrary code.
                    raise ValueError(
                        f"{hyperopt_file} is not a JSON hyperopt file. Legacy pickle "
                        "files are not supported; re-run hyperopt with a current freqtrade."
                    ) from e
    ...
    def _analyze_convergence(self, losses: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_loss_progression": (
                best_so_far[-50:] if len(best_so_far) > 50 else best_so_far
            ),  # Last 50 for visualization
            ...
            "improvement_percentage": (
                ((losses[0] - best_so_far[-1]) / abs(losses[0]) * 100)
            ...
            "avg_improvement_rate": (
                sum(improvement_rate) / len(improvement_rate) if improvement_rate else 0
            ),
        }
    ...
    def _create_optimization_summary(
        ...
        return {
            ...
            "optimization_efficiency": (
                self._calculate_optimization_efficiency(losses) if losses else 0
            ),
        }
    ...
    def _analyze_parameter_sensitivity(self, trial_results: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            try:
                if df[param].dtype in ["int64", "float64"]:
                    ...
                    correlations[param] = {
                        ...
                        "sensitivity": (
                            "High" if abs(corr) > 0.5 else "Medium" if abs(corr) > 0.2 else "Low"
                        ),
                    }
            except Exception:
        ...
        sorted_correlations = dict(
            sorted(correlations.items(), key=lambda x: x[1]["absolute_correlation"], reverse=True)
        )
            ...
            param_importance.append(
                {
                    ...
                    "interpretation": (
                        "Higher values increase loss"
                        if stats["correlation"] > 0
                        else "Higher values decrease loss"
                    ),
                }
            )
        ...
        return {
            ...
            "most_important_parameter": (
                param_importance[0]["parameter"] if param_importance else None
            ),
            "least_important_parameter": (
                param_importance[-1]["parameter"] if param_importance else None
            ),
            "high_sensitivity_params": [
                p for p, s in correlations.items() if s["sensitivity"] == "High"
            ],
        }
