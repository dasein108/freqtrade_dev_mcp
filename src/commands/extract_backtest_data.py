...
class ExtractBacktestDataCommand(BaseCommand):
            ...
            for file_name in file_list:
                try:
                    if file_name.endswith(".json"):
                        ...
                        elif file_name.endswith("_AlligatorFractalV2_Improved.json") or any(
                            strategy in file_name
                            for strategy in ["AlligatorFractalV2", "OLSPairsTrading"]
                        ):
                        ...
                        try:
                            ...
                            await self.mcp_log(
                                "warning", f"Failed to read feather file {file_name}: {e}"
                            )
    ...
    def _create_trades_summary(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "exit_reasons": dict(
                sorted(exit_reason_counts.items(), key=lambda x: x[1], reverse=True)
            ),
        }
    ...
    def _analyze_trades(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis["direction_performance"] = {
            "long_trades": {
                ...
                "avg_profit": (
                    sum(t.get("profit_ratio", 0) for t in long_trades) / len(long_trades)
                    if long_trades
                    else 0
                ),
                "winrate": (
                    len([t for t in long_trades if t.get("profit_ratio", 0) > 0]) / len(long_trades)
                    if long_trades
                    else 0
                ),
            },
            "short_trades": {
                ...
                "avg_profit": (
                    sum(t.get("profit_ratio", 0) for t in short_trades) / len(short_trades)
                    if short_trades
                    else 0
                ),
                "winrate": (
                    len([t for t in short_trades if t.get("profit_ratio", 0) > 0])
                    / len(short_trades)
                    if short_trades
                    else 0
                ),
        ...
        for trade in trades:
            ...
            if pair not in pair_performance:
                pair_performance[pair] = {"trades": 0, "profits": [], "durations": []}
        ...
        analysis["pair_performance"] = dict(
            sorted(pair_performance.items(), key=lambda x: x[1]["total_profit"], reverse=True)
        )
