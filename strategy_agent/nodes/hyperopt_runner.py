...
async def run_hyperopt(
    ...
    try:
        ...
        strategy_logger.log_data(
            logging.INFO,
            "Hyperopt configuration prepared",
            {
                "strategy": state["strategy_name"],
                "epochs": state["hyperopt_epochs"],
        ...
        logger.info(
            f"Calling MCP hyperopt_strategy with params: {json.dumps(hyperopt_config, indent=2)}"
        )
        ...
        if result and result.get("success"):
            ...
            if result:
                ...
                if is_critical:
                    strategy_logger.log_error(
                        "🚨 CRITICAL MCP SERVER ERROR DETECTED - HALTING EXECUTION",
            ...
            strategy_logger.log_error(
                f"Hyperopt failed: {error_msg}", details=json.dumps(error_details, indent=2)
            )
...
async def run_backtest_with_params(
    state: StrategyDevelopmentState, mcp_client: FreqtradeMCPClient
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result = await mcp_client.backtest_strategy(
            strategy=state["strategy_name"], **backtest_config
        )
        ...
        strategy_logger.log_error(
            "Failed to run backtest with optimized parameters", e, error_trace
        )
        return None
