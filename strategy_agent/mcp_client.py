...
from .models import BacktestResponse, DownloadCandlesResponse, HyperoptResponse, MCPResponse
...
class FreqtradeMCPClient:
                ...
                for content in result.content:
                    if hasattr(content, "text"):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Tool {tool_name} returned non-JSON content: {content.text[:200]}"
                            )
                            return {
                                "success": False,
    ...
    def _validate_response(
        self, tool_name: str, response_data: Dict[str, Any]
    ...
    async def download_candles(
        self, pairs: List[str], timeframe: str, days: int = 365, exchange: str = "binance"
        ...
        if is_futures:
            ...
            logger.info("Detected futures pairs, setting trading_mode=futures")
    ...
    async def extract_backtest_data(
        self, result_path: str, output_format: str = "summary"
    ) -> Dict[str, Any]:
        """
        ...
        return await self.call_tool(
            "extract_backtest_data", {"result_path": result_path, "output_format": output_format}
        )

    async def extract_hyperopt_data(
        self, hyperopt_path: str, output_format: str = "summary", max_trials: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Extracted data
        """
        params = {"hyperopt_path": hyperopt_path, "output_format": output_format}
    ...
    async def list_results(self, result_type: str = "backtest", limit: int = 10) -> Dict[str, Any]:
        """
        ...
        Returns:
            List of results
        """
        return await self.call_tool("list_results", {"result_type": result_type, "limit": limit})

    async def search_results(
        self, query: str, result_type: str = "backtest", limit: int = 10
    ) -> Dict[str, Any]:
        """
        Search for results
        ...
        """
        return await self.call_tool(
            "search_results", {"query": query, "result_type": result_type, "limit": limit}
        )
