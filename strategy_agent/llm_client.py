...
class SimpleLLMClient:
    ...
    def __init__(self, config: LLMConfig):
        if instructor is None:
            raise ImportError(
                "instructor package is required. Install with: pip install instructor"
            )
        ...
        if provider == "deepseek":
            ...
            base_client = AsyncOpenAI(api_key=config.api_key, base_url="https://api.deepseek.com")
            ...
            try:
                self.client = instructor.from_provider(model=config.model, api_key=config.api_key)
