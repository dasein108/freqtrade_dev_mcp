...
from .nodes.strategy_generator import create_strategy_code, generate_strategy_idea, rewrite_strategy
from .state import (
...
class StrategyDevelopmentAgent:
    ...
    async def develop_strategy(self, resume_from: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        strategy_logger.log_progress(
            f"Starting strategy development for {len(self.symbols)} symbols"
        )
        ...
        config = {
            "configurable": {"thread_id": initial_state["session_id"], "checkpoint_id": resume_from}
        }
        ...
        try:
            ...
            async with self.mcp_client:
                    ...
                    for node_name, node_state in event.items():
                        ...
                        if isinstance(node_state, dict):
                            ...
                            if node_state.get("errors"):
                                ...
                                error_summary = (
                                    f"Errors in state - {current_error_count} total error(s)"
                                )
                ...
                if final_state:
                    ...
                    return {"success": False, "error": "Workflow completed without final state"}
            ...
            results.update(
                {
                    ...
                    "metrics": {
                        ...
                        "win_rate": (
                            state["performance_metrics"].win_rate_pct
                            if state["performance_metrics"]
                            else 0
                        ),
                        "total_trades": (
...
async def main():
    ...
    agent = StrategyDevelopmentAgent(
        symbols=["BTC/USDT:USDT"], timeframes=["1h"], max_iterations=2, hyperopt_epochs=50
    )
