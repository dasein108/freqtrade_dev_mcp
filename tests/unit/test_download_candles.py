...
class TestDownloadCandlesCommand:
    ...
    async def test_execute_success(self, command):
        ...
        with patch.object(command, "_download_for_timeframe", return_value=mock_result):
            with patch(
                "src.commands.download_candles.parse_and_format_date",
                return_value="20240101-20250101",
            ):
                result = await command.execute(
                    pairs=["BTC/USDT:USDT"], timeframes=["1h"], date_range="last year"
                )
    ...
    async def test_process_pairs_top15_fallback(self, command):
        ...
        with patch.object(
            command.coingecko_client, "get_top_trading_pairs", side_effect=Exception("API error")
        ):
            result = await command._process_pairs("top15", "binance")
    ...
    def test_generate_cache_filename(self, command):
        ...
        filename = command._generate_cache_filename("BTC/USDT:USDT", "1h", "20240101-20250101")
    ...
    def test_generate_cache_filename_special_chars(self, command):
        ...
        filename = command._generate_cache_filename("BTC/USDT", "5m", "20240101-20240201")
