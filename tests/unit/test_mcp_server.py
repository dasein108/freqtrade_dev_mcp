...
class TestFreqtradeMCPServer:
    ...
    async def test_download_candles_tool_error(self, server):
        ...
        with patch.object(
            server.commands["download_candles"], "execute", return_value=error_result
        ):
            result = await server.commands["download_candles"].execute(
                pairs="BTC/USDT:USDT", timeframes="1h", date_range="last year"
            )
    ...
    async def test_tool_with_invalid_json_response(self, server):
        ...
        with patch.object(
            server.commands["download_candles"], "execute", return_value={"test": set([1, 2, 3])}
        ):
            result = await server.commands["download_candles"].execute(
                pairs="BTC/USDT:USDT", timeframes="1h", date_range="last year"
            )
    ...
    async def test_mcp_logging_integration(self, server):
        ...
        with patch.object(server, "send_log_message"):
            # Execute a command that should generate logs
            mock_result = {"command": "test", "success": True}

            with patch.object(
                server.commands["download_candles"], "execute", return_value=mock_result
            ):
