...
from src.models.base_models import CacheFileInfo, DownloadCandlesResponse
...
class TestMCPIntegration:
    ...
    async def test_download_candles_workflow(self, mcp_server, temp_data_dir, sample_candle_data):
        ...
        with patch.object(
            mcp_server.commands["download_candles"],
            "execute",
            return_value=download_response.model_dump(),
        ):
            # Call download_candles
            download_result = await mcp_server.commands["download_candles"].execute(
                pairs="BTC/USDT:USDT", timeframes=["1h"], date_range="20240101-20250101"
            )
    ...
    async def test_schema_validation_consistency(self, mcp_server, temp_data_dir):
        ...
        with patch.object(
            ...
            download_result = await mcp_server.commands["download_candles"].execute(
                pairs="ETH/USDT", timeframes=["4h"], date_range="20240101-20240201"
            )
    ...
    async def test_error_handling_consistency(self, mcp_server):
        ...
        with patch.object(
            mcp_server.commands["download_candles"], "execute", return_value=error_response
        ):
            download_result = await mcp_server.commands["download_candles"].execute(
                pairs="INVALID/PAIR", timeframes=["1h"], date_range="invalid"
            )
    ...
    async def test_multiple_pairs_download(self, mcp_server, temp_data_dir, sample_candle_data):
        ...
        with patch.object(
            ...
            download_result = await mcp_server.commands["download_candles"].execute(
                pairs=pairs, timeframes=["1h"], date_range="20240101-20240201"
            )
    ...
    async def test_json_serialization_consistency(self, mcp_server, temp_data_dir):
        ...
        mock_cache_info = CacheFileInfo(
            pair="TEST/PAIR", filename=cache_filename, full_path=str(cache_path), candle_count=1
        )
        ...
        with patch.object(
            ...
            download_result = await mcp_server.commands["download_candles"].execute(
                pairs="TEST/PAIR", timeframes=["1h"], date_range="20240101-20240201"
            )
