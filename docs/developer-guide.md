...
> **Note:** the `read_candles` tool described in parts of this guide has been removed. `download_candles` returns cache file paths (`cache_files[].full_path`) and clients read those Freqtrade JSON files directly; see `read_ohlcv_cache_file` in `strategy_agent/nodes/data_fetcher.py`. The current tool list is in [tools-reference.md](tools-reference.md).
